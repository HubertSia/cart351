...
@app.route("/three")
def three():
    # Select all entries where after_mood is positive
    results = mongo.db.dataStuff.find({"after_mood": {"$in": positive_moods}})
    # Return in the same structure used by routes one and two
    return {"results": results, "moods": positive_moods}


@app.route("/four")
def four():
    # Select all entries, sorted by event_name
...
@app.route("/five")
def five():
    # Select Monday or Tuesday, sorted by event_affect_strength
    results = mongo.db.dataStuff.find({"day": {"$in": ["Monday", "Tuesday"]}}).sort(
        "event_affect_strength", 1
    )
...
@app.route("/six")
def six():
    # Entries where both start_mood and after_mood fall into the negative category
    results = mongo.db.dataStuff.find(
        {
    ...
    return {"results": results, "moods": negative_moods}
